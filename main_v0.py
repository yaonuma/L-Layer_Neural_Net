...
def resize(images, dims):

    """
    Resize images

    :param images: <ndarray <float>> image data to transform
...
def image_preprocess(all_images):
    ...
    :return: <ndarray <float>> preprocessed images in compatible form for model training
    """

    images = resize(all_images, (64, 64))
...
def compute_cost(AL, Y, parameters, _lambda):
    ...
    # # Compute loss from aL and y. (without regularization)
    # cost = (1. / m) * (-np.dot(Y, np.log(AL).T) - np.dot(1 - Y, np.log(1 - AL).T))

    # cost = np.squeeze(cost)  # To make sure your cost's shape is what we expect
    # assert (cost.shape == ())

    # # Cost with L2 regularization. L2 regularization penalizes the squared values of the weights in the
    # # cost function so that it becomes costly to have large weights. This has the effect of smoothing the model
    # m = Y.shape[1]
    # cross_entropy_cost = (-np.dot(Y, np.log(AL).T) - np.dot(1 - Y, np.log(1 - AL).T))
    # l2_regularization_cost = np.sum([np.sum(np.square(v)) for k, v in parameters.items() if 'W' in k]) * 1 / m * _lambda / 2
    ...
    m = Y.shape[1]
    cross_entropy_cost = (-np.dot(Y, np.log(AL).T) - np.dot(1 - Y, np.log(1 - AL).T))
    l2_regularization_cost = np.sum(
        [np.sum(np.square(v)) for k, v in parameters.items() if 'W' in k])  * _lambda / 2
    cost = cross_entropy_cost + l2_regularization_cost
    cost = np.squeeze(cost)
    assert (cost.shape == ())
...
def linear_backward(dZ, cache, _lambda):
    ...
    :param _lambda: L2 regularization regularization rate
...
def linear_activation_backward(dA, cache,  _lambda, activation):
    ...
    if activation == "relu":
        ...
        dA_prev, dW, db = linear_backward(dZ, linear_cache, _lambda)
...
def L_model_backward(AL, Y, caches, _lambda):
    ...
    grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = \
                                           linear_activation_backward(dAL, current_cache, _lambda, "sigmoid")

    for l in reversed(range(L - 1)):
        # lth layer: (RELU -> LINEAR) gradients.
        current_cache = caches[l]
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward(grads["dA" + str(l + 2)], current_cache, _lambda,
                                                                    "relu")
...
def update_parameters(parameters, grads, learning_rate, v, beta):
...
def build_layers(train_x, best_params):

    layers_dims = [np.shape(train_x)[0]]
    hl = sorted([item[1] for item in best_params.items() if "hl" in item[0]], reverse=True)
    layers_dims = layers_dims + hl
    layers_dims.append(1)

    return layers_dims


def random_mini_batches(X, Y, mini_batch_size=32, seed=None):
    """
    Creates a list of random minibatches from (X, Y). Consists of two main steps, shuffling and partitioning
    ...
    :param seed: integer seed the random generator, like in np.random
...
def L_layer_model(X, Y, layers_dims, mini_batch_size=32, learning_rate=0.0075, num_epochs=2400, print_cost=False,
                  print_learning_rate=True, _lambda=0.1, beta=0.9):
    ...
    for i in range(0, num_epochs):
        ...
        if seed != None:
            seed = seed + 1
        minibatches = random_mini_batches(X, Y, mini_batch_size=32, seed=seed)
        cost_total = 0
        ...
        for minibatch in minibatches:
            ...
            # Compute cost
            cost_total += compute_cost(AL, minibatch_Y, parameters, _lambda)

            # Backward propagation
            grads = L_model_backward(AL, minibatch_Y, caches, _lambda)
            # grads = L_model_backward(minibatch_X, minibatch_Y, caches, _lambda)

            # Update parameters.
            parameters, v = update_parameters(parameters, grads, learning_rate, v, beta)
...
if __name__ == '__main__':
    ...
    # best_params = {"hl_0": 18, "hl_1": 16, "hl_2": 12, "hl_3":5, "hl_4":3, "learning_rate": 0.0010}
    # best_params = {"hl_0": 9, "hl_1": 5, "hl_2": 3, "learning_rate": 0.00050}
    best_params = {"hl_0": 20, "hl_1": 7, "hl_2": 5, "learning_rate": 0.00750}

    # get optimal params (maybe)
    optimize_best_params = 0
    if optimize_best_params:
        ...
        layer_hidden_units = [
                              range(48, 64),  # hidden layer 1 possible number of hidden units
                              range(36, 48),  # hidden layer 1 possible number of hidden units
                              range(12, 36), # hidden layer 1 possible number of hidden units
                              range(6, 12),  # hidden layer 2 possible number of hidden units
        ...
        for i in range(N):
            ...
            hl_3 = random.choice(layer_hidden_units[2])
            hl_4 = random.choice(layer_hidden_units[2])
            ...
            for train_index, validation_index in skf.split(train_x_orig, train_y.T):
                ...
                # layers_dims = [np.shape(X_train)[0], hl_0, hl_1, hl_2, hl_3, hl_4, 1]  # 4-layer model
                layers_dims = build_layers(X_train, best_params)
                parameters = L_layer_model(X_train, y_train, layers_dims, learning_rate=learning_rate,
                                           num_epochs=2400, print_cost=True, print_learning_rate=False, _lambda=0.1)
            ...
            if roc_avg > best_auc_avg:
                ...
                best_params["hl_3"] = hl_3
                best_params["hl_4"] = hl_4
                best_params["learning_rate"] = learning_rate
        print('\nBest ROC_AUC average after', N,'iterations of KFold Cross Validations:', best_auc_avg)
        print('Best parameters: ', best_params,'\n')
        end = time.time()
        printm('\nFinished randomized search CV in ' + str(round((end-start)/60, 3)) +
               ' minutes.\n/n', color='magenta', switch=TERMCOLOR)

    # use best params now
    printm('Using user-defined hyperparams on the whole training set ...', color='red', switch=TERMCOLOR)
    ...
    # layers_dims = [np.shape(train_x)[0], best_params["hl_0"], best_params["hl_1"], best_params["hl_2"],
    #                best_params["hl_3"], best_params["hl_4"], 1]  # 6-layer model
    layers_dims = [np.shape(train_x)[0], best_params["hl_0"], best_params["hl_1"], best_params["hl_2"], 1]  # 6-layer model

    # layers_dims = [np.shape(train_x[0])[0]]
    # hl = sorted([item[1] for item in best_params.items() if "hl" in item[0]], reverse=True)
    # layers_dims = layers_dims + hl
    # layers_dims.append(1)

    layers_dims = build_layers(train_x, best_params)
    parameters = L_layer_model(train_x, train_y, layers_dims, mini_batch_size=train_x.shape[1], learning_rate=best_params["learning_rate"],
                               num_epochs=2400, print_cost = True, _lambda=0, beta=0)
